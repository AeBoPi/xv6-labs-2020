...
// kvm_map_pagetable()函数，可以为任何我们自己创建的内核页表添加映射
...
void kvm_map_pagetable(pagetable_t pgtbl) {
  ...
  // CLINT
  // kvmmap(pgtbl, CLINT, CLINT, 0x10000, PTE_R | PTE_W);  // lab3.3这里注释掉
...
kvminit()
{
  ...
  // CLINT
  kvmmap(kernel_pagetable, CLINT, CLINT, 0x10000, PTE_R | PTE_W);  // lab3.3 在全局内核页表加上 CLINT 映射
...
{
  if(mappages(pgtbl, va, sz, pa, perm) != 0)   // modified kernel_pagetable->pgtbl
    panic("kvmmap");
}
...
// kvmpa 将内核逻辑地址转换为物理地址（添加第一个参数 pgtbl）
uint64
...
int kvmcopymappings(pagetable_t src, pagetable_t dst, uint64 start, uint64 sz)
{
  ...
  // PGROUNDUP: prevent re-mapping already mappd pages (eg. when doing growproc)
